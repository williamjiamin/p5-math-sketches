/* jshint esversion: 6 */
// ============================================================
// 23. Subtraction on the Number Line — Animated Backward Hops
// ============================================================
...
function setup() {
  ...
  COL_CHAR      = color(60, 140, 220);
  COL_HOP       = color(220, 80, 100);
...
function drawTitle() {
  ...
  text("Subtraction on the Number Line", canvasW / 2, 30);
}
...
function drawHops() {
  ...
  for (var i = 0; i < completedHops.length; i++) {
    var sn = numA - i;
    drawHopArc(numToX(sn), numToX(sn - 1), NL_Y, hopArcH, COL_HOP, 180, i + 1);
  }
...
function drawHopArc(sx, ex, baseY, h, col, alphaVal, label) {
  noFill();
  ...
  fill(col);
  noStroke();
  var arrowDir = (ex < sx) ? -1 : 1;
  triangle(
    midX + arrowDir * 6, baseY - h - 12,
    midX - arrowDir * 3, baseY - h - 8,
    midX - arrowDir * 3, baseY - h - 16
  );

  textAlign(CENTER, CENTER);
  textSize(Math.min(12, canvasW * 0.018));
  text(label, midX, baseY - h - 24);
}
...
function drawInstructions() {
  ...
  text("Click anywhere to start the hops!", canvasW / 2, canvasH * 0.35);
}
