...
// Zoom / camera (world -> screen)
let viewScale = 1;
let viewOffsetX = 0;
let viewOffsetY = 0;
const minViewScale = 0.4;
...
function draw() {
  ...
  // Draw everything in "world space" so zoom works
  push();
...
function updateDot() {
  ...
  let newX = constrain(worldMouseX(), axisStartX, axisEndX);
...
function mouseWheel(event) {
  // Zoom to cursor: keep the world point under the mouse fixed
  const oldScale = viewScale;
  const zoomIntensity = 0.0015; // smaller = slower zoom
  const zoomFactor = exp(-event.delta * zoomIntensity);

  const newScale = constrain(oldScale * zoomFactor, minViewScale, maxViewScale);
  if (newScale === oldScale) return false;

  const wx = (mouseX - viewOffsetX) / oldScale;
  const wy = (mouseY - viewOffsetY) / oldScale;

  viewScale = newScale;
  viewOffsetX = mouseX - wx * newScale;
  viewOffsetY = mouseY - wy * newScale;
