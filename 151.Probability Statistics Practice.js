...
function newProblem(){answered=-1;var t=Math.floor(Math.random()*4);
  if(t===0){prob={q:"68-95-99.7 rule: What % within ±1σ?",ans:"68%"};options=["68%","95%","99.7%"];}
  else if(t===1){prob={q:"P(A|B) = P(B|A)P(A)/P(B) is called?",ans:"Bayes' Theorem"};options=["Bayes' Theorem","Law of Large Numbers","CLT"];}
  else if(t===2){var lam=Math.floor(Math.random()*5)+2;prob={q:"Poisson(λ="+lam+"): E[X] = ?",ans:""+lam};options=[""+lam,""+(lam*2),""+(lam-1)];}
  else{prob={q:"CLT says sample means are approx. ___?",ans:"Normal"};options=["Normal","Uniform","Exponential"];}
  for(var i=options.length-1;i>0;i--){var j=Math.floor(Math.random()*(i+1));var tmp=options[i];options[i]=options[j];options[j]=tmp;}}
function draw(){background(245,247,252);noStroke();fill(60,40,120);textSize(22);textStyle(BOLD);text("Probability & Statistics Practice",width/2,25);textStyle(NORMAL);
  fill(30,50,120);textSize(15);text(prob.q,width/2,65);
  var optW=150,optH=40,gap=15,startX=width/2-(options.length*optW+(options.length-1)*gap)/2;
