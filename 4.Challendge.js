...
let dotX;                 // Starting position (center of axis)
...
let axisStartX;
let axisEndX;
let minValue = 0;        // Minimum value on axis
...
function setup() {
  ...
  updateAxisLayout();
  dotX = map(5, minValue, maxValue, axisStartX, axisEndX);
}
...
function displayNumber() {
  ...
  let decimalPart = (value - integerPart).toFixed(2);
  ...
  textSize(20);
  textStyle(NORMAL);
  let decimalText = "." + decimalPart.substring(2);
  ...
  textSize(28);
  textStyle(BOLD);
  text(integerPart, startX + integerWidth / 2, axisY - 120);
