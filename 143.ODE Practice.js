...
function newProblem(){answered=-1;var t=Math.floor(Math.random()*3);
  if(t===0){prob={q:"y'=2y, y(0)=3. Solution?",ans:"3e^(2t)"};options=["3e^(2t)","2e^(3t)","3+2t"];}
  else if(t===1){prob={q:"y''+4y=0 has solutions of form?",ans:"sin(2t), cos(2t)"};options=["sin(2t), cos(2t)","e^(2t), e^(-2t)","t, t²"];}
  else{prob={q:"Euler step: y₁ = y₀ + h·f(x₀,y₀). If h=0.1, y₀=1, f=y, then y₁ = ?",ans:"1.1"};options=["1.1","1.01","0.9"];}
  for(var i=options.length-1;i>0;i--){var j=Math.floor(Math.random()*(i+1));var tmp=options[i];options[i]=options[j];options[j]=tmp;}}
function draw(){background(245,247,252);noStroke();fill(60,40,120);textSize(22);textStyle(BOLD);text("ODE Practice",width/2,25);textStyle(NORMAL);
